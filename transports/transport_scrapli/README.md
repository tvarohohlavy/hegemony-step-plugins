...
scrapli core platforms (IOS-XE, IOS-XR, NX-OS, EOS, Junos). Implements the SDK
`Transport` command surface (`execute_command(s)`, `execute_command_timing`);
file staging (`scp_put`, `http_transfer`) is not supported by this transport
and raises `NotImplementedError` — use the netmiko transport for staging steps.
